...
type StudioTab = 'editor' | 'terminal' | 'preview' | 'packages' | 'env' | 'git' | 'search' | 'quality' | 'history' | 'templates' | 'docker' | 'database' | 'api' | 'debug';
const TAB_ORDER: StudioTab[] = ['editor', 'terminal', 'preview', 'packages', 'env', 'git', 'search', 'quality', 'history', 'templates', 'docker', 'database', 'api', 'debug'];
...
export function BuildStudio({ open, onClose, title, initialCommands, onRefreshFiles, initialPrompt, runKey }: BuildStudioProps) {
  ...
  const openFileAtLine = async (path: string, line: number) => {
    await openFile(path);
    // Scroll to line after file is loaded - use a small delay to ensure editor is ready
    setTimeout(() => {
      const editor = document.querySelector('.code-editor .cm-editor') as HTMLElement;
      if (editor) {
        const view = editor['CodeMirror'] || editor.cmView;
        if (view && view.state) {
          const linePos = view.state.doc.line(Math.min(line, view.state.doc.lines)).from;
          view.dispatch({ selection: { anchor: linePos }, scrollIntoView: true });
          view.focus();
        }
      }
    }, 50);
  ...
  const toggleAfkMode = async () => {
    ...
    setAfkMode(enabling);
    setNotice(enabling
      ? (previewRunning && lastPrompt ? 'AFK mode enabled. Infinity is continuing the local build review.' : 'AFK mode enabled. Start a preview to let Infinity continue autonomously.')
      : 'AFK mode disabled');
  ...
  useEffect(() => {
    ...
    const wake = (event: Event) => {
      ...
      setDeepSleep(false);
      setSleepSummary(afkMode ? 'Infinity woke from AFK mode. Review the activity feed and checkpoint before continuing.' : 'Sleep screen dismissed.');
    };
  ...
  if (!open) return null;
  const renderSleepControlsLegacy = () => <div data-build-sleep-controls="true" aria-label="AFK and sleep controls" className="flex items-center gap-1.5"><button type="button" onClick={() => void toggleAfkMode()} disabled={historyBusy} className={`rounded-lg border px-2.5 py-2 text-[11px] ${afkMode ? 'border-amber-300/50 bg-amber-300/10 text-amber-200' : 'border-border/50 text-muted-foreground hover:bg-secondary/60'}`} title="Let Infinity continue the local build loop"><Zap className="mr-1 inline h-3.5 w-3.5" />{afkMode ? 'AFK on' : 'AFK mode'}</button><select value={wakeSensitivity} onChange={(event) => setWakeSensitivity(event.target.value as 'any' | 'keypress')} className="hidden rounded-lg border border-border/50 bg-secondary/40 px-2 py-2 text-[11px] outline-none sm:block" aria-label="Sleep wake sensitivity"><option value="any">Wake: any input</option><option value="keypress">Wake: keypress</option></select><button type="button" onClick={() => setDeepSleep(true)} className="rounded-lg border border-border/50 px-2.5 py-2 text-[11px] text-muted-foreground hover:bg-secondary/60" title="Dim the interface while Infinity works"><Moon className="mr-1 inline h-3.5 w-3.5" />Sleep</button></div>;
  const renderDeepSleepOverlay = deepSleep ? <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black text-white" role="dialog" aria-label="Infinity sleep screen"><Moon className="h-8 w-8 text-white/30" /><p className="mt-4 text-xs tracking-[0.25em] text-white/40">INFINITY SLEEP SCREEN</p>{afkMode && <p className="mt-3 text-[11px] text-white/25">⚡ Agent working · {autoFixPass > 0 ? `iteration ${autoFixPass}` : 'checkpoint saved'}</p>}</div> : null;
  ...
  const tabs: [StudioTab, string, typeof Code2][] = [['editor', t('studio.build.editor'), Code2], ['terminal', t('studio.build.terminal'), Terminal], ['preview', t('studio.build.preview'), Play], ['debug', t('studio.build.debug'), Bug], ['packages', t('studio.build.packages'), Package], ['env', t('studio.build.env'), Upload], ['git', t('studio.build.git'), GitBranch], ['search', 'Search', Search], ['quality', 'Quality', TestTube2], ['history', 'History', History], ['templates', 'Templates', LayoutTemplate], ['docker', 'Docker', Container], ['database', 'Database', Database], ['api', 'API', Globe]];
  // Keep the studio calm: Editor/Terminal/Preview/Debug always visible, the rest
  // tucked behind a "More" toggle so a fresh build isn't a wall of 13 tabs.
  const coreTabs = tabs.slice(0, 4);
...
{tab === 'editor' && <div className="flex h-[calc(100%-49px)] min-h-0 flex-col"><div className="flex shrink-0 items-center gap-1 overflow-x-auto border-b border-border bg-card px-3 py-2">{openPaths.map((path) => <button type="button" key={path} onClick={() => void openFile(path)} className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[11px] ${selectedPath === path ? 'bg-background text-white' : 'text-muted-foreground'}`}>{path}{selectedPath === path && dirty ? ' ·' : ''}</button>)}<button type="button" onClick={() => void createFile()} className="rounded-lg p-1.5 text-muted-foreground hover:bg-white/[0.08] hover:text-foreground"><Plus className="h-3.5 w-3.5" /></button></div>{selectedPath ? <><div className="flex items-center justify-between border-b border-border bg-background px-3 py-2 text-[10px] text-muted-foreground"><span className="text-foreground">{selectedPath} <span className="text-muted-foreground">{languageFor(selectedPath)}{dirty ? ' · unsaved' : ''}</span></span><button type="button" disabled={!dirty || busy} onClick={() => void saveFile()} className="flex items-center gap-1 rounded-lg bg-primary/20 px-2.5 py-1.5 text-primary disabled:opacity-40"><Save className="h-3 w-3" />Save</button></div><div className="min-h-0 flex-1 overflow-hidden"><CodeEditor value={content} path={selectedPath} onChange={(value) => { setContent(value); setDirty(true); }} onCursorChange={(line, col) => setCursorPos({ line, col })} onSave={() => void saveFile()} cursorConfig={{ projectId: workspaceId, projectRoot: `/workspace/${workspaceId}`, filePath: selectedPath, tabAutocompleteEnabled: true, onNavigate: (filePath, line) => void openFileAtLine(filePath, line) }} /></div><div className="flex shrink-0 items-center gap-3 border-t border-border bg-primary px-3 py-1.5 text-[10px] text-white/80"><span className="font-medium text-white">{languageFor(selectedPath)}</span><span className="font-mono">{cursorPos ? `Ln ${cursorPos.line}, Col ${cursorPos.col}` : '—'}</span><span className="font-mono">{new Blob([content]).size.toLocaleString()} B</span><span className={dirty ? 'text-amber-300' : 'text-white'}>{dirty ? 'unsaved' : 'saved'}</span><span className="ml-auto hidden gap-2 sm:flex text-white/60">Ctrl+S save · Ctrl+J terminal · Ctrl+1–9 tabs</span></div></> : <div className="flex h-full flex-col items-center justify-center gap-4 p-6 text-center"><Code2 className="h-10 w-10 text-primary/50" /><div><p className="text-sm font-semibold text-foreground">{t('studio.build.openFile')}</p><p className="mt-1 max-w-md text-xs text-muted-foreground">{t('studio.build.openFileDesc')}</p></div><div className="w-full max-w-md space-y-2"><input ref={scaffoldInputRef} value={scaffoldPrompt} onChange={(event) => setScaffoldPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void beginScaffold(); }} placeholder={t('studio.build.scaffoldPlaceholder')} className="min-w-0 w-full rounded-xl border border-white/[0.12] bg-input px-3 py-2.5 text-xs text-foreground outline-none placeholder:text-muted-foreground" /><textarea value={extraSystemPrompt} onChange={(event) => setExtraSystemPrompt(event.target.value)} placeholder={t('studio.build.extraInstructionsPlaceholder')} rows={3} className="w-full resize-y rounded-xl border border-white/[0.12] bg-secondary px-3 py-2.5 text-xs text-foreground outline-none placeholder:text-muted-foreground" /><button type="button" onClick={() => void beginScaffold()} disabled={wizardBusy || !scaffoldPrompt.trim()} className="rounded-xl bg-primary px-3 py-2 text-xs font-medium text-white disabled:opacity-50">{t('studio.build.scaffold')}</button></div></div>}</div>}
        ...
        {tab === 'packages' && <div className="h-[calc(100%-49px)] space-y-4 overflow-auto p-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input value={packageQuery} onChange={(event) => setPackageQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void searchPackages(); }} placeholder={t('studio.build.packageSearchPlaceholder')} className="min-w-0 flex-1 rounded-lg border border-border bg-input px-2.5 py-2 text-xs text-foreground outline-none placeholder:text-muted-foreground" />
          ...
          {gitStatus ? <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-3">
              <div className="rounded-xl border border-border bg-secondary p-3">
                ...
                <div className="flex gap-2"><input value={commitMessage} onChange={(event) => setCommitMessage(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void commitChanges(); }} placeholder={t('studio.build.commitMessagePlaceholder')} className="min-w-0 flex-1 rounded-lg border border-border bg-input px-2.5 py-2 text-xs text-foreground outline-none placeholder:text-muted-foreground" /><button type="button" disabled={gitBusy || !commitMessage.trim()} onClick={() => void commitChanges()} className="flex items-center gap-1 rounded-lg bg-primary px-3 py-2 text-[11px] font-medium text-white disabled:opacity-50"><GitCommit className="h-3 w-3" />Commit</button></div>
              </div>
              ...
              {gitBranches.length > 0 && <div className="rounded-xl border border-border bg-secondary p-3"><p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Branches</p><div className="flex flex-wrap gap-1.5">{gitBranches.map((branch) => <button key={branch} type="button" onClick={() => void switchBranch(branch)} className={'rounded-lg px-2 py-1 text-[10px] font-mono ' + (branch === gitCurrentBranch ? 'bg-primary/20 text-primary' : 'bg-input/50 text-muted-foreground hover:text-foreground')}>{branch}</button>)}</div><div className="mt-2 flex gap-2"><input value={newBranchName} onChange={(event) => setNewBranchName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void createBranch(); }} placeholder={t('studio.build.newBranchPlaceholder')} className="min-w-0 flex-1 rounded-lg border border-border bg-input px-2.5 py-1.5 text-xs text-foreground outline-none placeholder:text-muted-foreground" /><button type="button" disabled={!newBranchName.trim()} onClick={() => void createBranch()} className="shrink-0 rounded-lg bg-primary/20 px-2.5 py-1.5 text-[11px] font-medium text-primary">Create</button></div></div>}
            </div>
        ...
        {tab === 'search' && <div className="h-[calc(100%-49px)] space-y-4 overflow-auto p-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void runWorkspaceSearch(); }} placeholder={t('studio.build.workspaceSearchPlaceholder')} className="min-w-0 flex-1 rounded-lg border border-border bg-input px-2.5 py-2 text-xs text-foreground outline-none placeholder:text-muted-foreground" />
            <input value={replaceText} onChange={(event) => setReplaceText(event.target.value)} placeholder={t('studio.build.replacePlaceholder')} className="w-44 rounded-lg border border-border bg-input px-2.5 py-2 text-xs text-foreground outline-none placeholder:text-muted-foreground" />
        ...
        {tab === 'quality' && <div className="h-[calc(100%-49px)] space-y-4 overflow-auto p-5">
          <div className="flex items-center gap-3"><TestTube2 className="h-6 w-6 text-primary" /><div><h3 className="text-sm font-semibold text-foreground">Quality: tests & debugging</h3><p className="mt-0.5 text-xs text-muted-foreground">Run detected test frameworks and analyze preview errors with Infinity.</p></div></div>
        ...
        {tab === 'database' && <div className="h-[calc(100%-49px)] space-y-4 overflow-auto p-5">
          ...
          <div className="rounded-xl border border-border bg-secondary p-3"><p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">SQL</p><div className="flex gap-2"><input value={dbQuery} onChange={(event) => setDbQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void runDbQuery(); }} placeholder={t('studio.build.dbQueryPlaceholder')} className="min-w-0 flex-1 rounded-lg border border-border bg-background px-2.5 py-2 font-mono text-xs text-foreground outline-none placeholder:text-muted-foreground" /><button type="button" disabled={dbBusy || !dbQuery.trim()} onClick={() => void runDbQuery()} className="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-50">{dbBusy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Play className="h-3 w-3" />}Run</button></div></div>
        ...
        {tab === 'api' && <div className="h-[calc(100%-49px)] space-y-4 overflow-auto p-5">
          ...
          <div className="rounded-xl border border-border bg-secondary p-3"><p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Request</p><div className="flex flex-wrap gap-2"><select value={apiMethod} onChange={(event) => setApiMethod(event.target.value)} className="rounded-lg border border-border bg-input px-2 py-2 text-xs text-foreground outline-none">{['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].map((m) => <option key={m}>{m}</option>)}</select><input value={apiBaseUrl} onChange={(event) => setApiBaseUrl(event.target.value)} placeholder={t('studio.build.apiBaseUrlPlaceholder')} className="w-52 rounded-lg border border-border bg-input px-2.5 py-2 text-xs text-foreground outline-none placeholder:text-muted-foreground" /><input value={apiEndpointPath} onChange={(event) => setApiEndpointPath(event.target.value)} placeholder={t('studio.build.apiEndpointPlaceholder')} className="min-w-0 flex-1 rounded-lg border border-border bg-input px-2.5 py-2 font-mono text-xs text-foreground outline-none placeholder:text-muted-foreground" /><button type="button" disabled={apiBusy} onClick={() => void sendApiRequest()} className="flex items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-xs font-medium text-white disabled:opacity-50">{apiBusy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}Send</button></div><textarea value={apiBody} onChange={(event) => setApiBody(event.target.value)} placeholder={t('studio.build.apiBodyPlaceholder')} rows={2} className="mt-2 w-full resize-y rounded-lg border border-border bg-background p-2 font-mono text-xs text-foreground outline-none placeholder:text-muted-foreground" /></div>
        ...
        {tab === 'debug' && <div className="h-[calc(100%-49px)] overflow-hidden"><BuildDebugPanel workspaceId={workspaceId} /></div>}
