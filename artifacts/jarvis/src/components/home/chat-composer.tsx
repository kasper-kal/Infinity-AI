...
export function ChatComposer(props: ChatComposerProps) {
  ...
  return (
    <div
      data-chat-composer
      className={`ios-composer relative flex-shrink-0 space-y-2 px-3 sm:px-4 pt-3 ${dragOver ? "is-dragging" : ""}`}
      style={{
    ...
    >
      ...
      <AnimatePresence>
        {dragOver && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-3 z-20 rounded-[2rem] border-2 border-dashed border-primary/45 bg-primary/10 flex items-center justify-center pointer-events-none backdrop-blur-xl"
          >
            <p className="font-display text-sm tracking-widest text-primary/70">
              {t("input.dropHere")}
            </p>
          </motion.div>
        )}
      ...
      {attachedFile && (
        <div className="mx-auto flex w-full max-w-3xl items-center gap-2 rounded-[1.5rem] border border-border/50 bg-background/75 p-2 shadow-apple-sm backdrop-blur-xl">
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-mono text-foreground/80 tracking-widest truncate">
              {attachedFile.fileName}
            </p>
            <p className="text-[10px] font-mono text-muted-foreground/50 tracking-widest">
              {t("input.fileAttachedLabel")}
            </p>
          </div>
        </div>
      )}
      <div className="ios-composer-bar mx-auto flex w-full max-w-3xl items-end gap-1.5 px-2 py-2 sm:px-2.5">
        {/* Hidden file inputs */}
        <input
          ref={fileInputRef}
          type="file"
        ...
        <div className="relative flex-shrink-0" ref={plusButtonRef}>
          <button
            id="plus-menu-button"
            onClick={() =>
              plusMenuOpen ? onClosePlusMenu() : onOpenPlusMenu()
            }
            disabled={isBusy}
            title={t("input.attachTitle")}
            className={`ios-tool-button p-2 rounded-full transition-all disabled:opacity-30 ${
        ...
        <button
          ...
          className={`ios-tool-button p-2 rounded-full transition-all flex-shrink-0 disabled:opacity-30 ${
            thinkingEnabled
              ? "text-primary bg-primary/10"
              : "text-foreground/70 hover:text-foreground hover:bg-secondary/70"
          }`}
            ...
            placeholder={
              chatDictating
                ...
                : isBusy
                  ? t("input.processing")
                  : attachedFile
                    ? t("input.placeholderFile")
                    : t("input.placeholder")
            }
            disabled={isBusy}
            className="chat-composer-input w-full bg-transparent text-foreground placeholder:text-muted-foreground/55 font-sans text-[15px] leading-6 pl-2 pr-12 py-2.5 outline-none resize-none min-h-[44px] max-h-[150px]"
          />
          <button
            ...
            aria-label={
              chatDictating ? t("input.stopDictate") : t("input.dictate")
            }
        ...
        <button
          ...
          className="ios-voice-button w-10 h-10 rounded-full bg-foreground text-background flex items-center justify-center shadow-md hover:opacity-90 active:scale-95 transition-all flex-shrink-0 disabled:opacity-30 disabled:cursor-not-allowed"
        >
          <span
            aria-hidden="true"
      ...
      {agentModeActive && (
        <div className="mx-auto flex max-w-3xl items-center gap-1.5 px-3 pb-1">
          <Search className="w-3 h-3 text-primary" />
          <span className="text-[11px] font-mono text-primary tracking-wider">
            {t("input.agentModeIndicator")}
          </span>
        </div>
      )}
