LAST_UPDATED: 2026-09-12 — **PHASE G COMPLETE — Error Feedback Loop (R1+R2). ALL G.1–G.5 DONE.** Failures become the next goal; the loop repairs and re-verifies. **G.1** (carried by C/D): verify-after-edit feeds `verification_failure` into the agent conversation; iterate pre-verifies and injects `## REAL VERIFICATION FAILURES TO FIX` into the goal — no sleep, no blind retry. **G.2** — after `tryLocalModelFix` applies patches, `runVerification` re-runs and the post-fix `verification_after_fix` result lands in `state.toolResults` + conversation + `## ERRORS SO FAR`; still-failing → `state.errors`. **G.3** — `computeWorkspaceFingerprint` (sorted edited files + tool-call signatures) over a rolling 6-iteration window; ≥2 identical fingerprints inject `"No progress detected — stop repeating and pick a different approach"` as an error + `oscillation_detected` tool message + telemetry. **G.4** — errors first-class: `## ERRORS SO FAR (verification failures · oscillation warnings · tool errors)`. **G.5** (carried by A 0.10): zero `|| true` in the verification pipeline (verified; only unrelated CORS fallback remains). Build clean — esbuild 3.4s; build-agent.ts typechecks clean. PHASES.md Phase G ✅ COMPLETE. Next: Phase H (Preview + Workspace Context). — Earlier: PHASE F COMPLETE (The Model Sees Bytes, 3.1–3.3 + 7.1–7.20), PHASE E COMPLETE (green must mean something, failure honesty), PHASE D COMPLETE (world re-enters, false-green killed, DOM to iterate), PHASE C COMPLETE (loop is one mind, 6/6 counters), PHASE B COMPLETE (real workspaces + scaffold engine), PHASE A COMPLETE (10 env fixes), all 42 PHASES COMPLETE.
...
4d. **✅ DONE — PHASE G COMPLETE — Error Feedback Loop** — G.1 no sleep/retry (carried), G.2 re-verify after tryLocalModelFix (post-fix `verification_after_fix` into toolResults + conversation), G.3 oscillation detection (rolling 6-iteration fingerprints → "No progress detected" error), G.4 errors first-class, G.5 no `|| true` (carried). Build clean (esbuild 3.4s; build-agent.ts tsc clean). **READY TO START — Phase H: Preview + Workspace Context** — per PHASES.md: H.1 screenshot to model, H.2 DOM to model (preview/agent structured output), H.3 headless Chrome deps (sudo apt infra), H.4 workspace context every call, H.5 project conventions, H.6 file tree persistence. Requires user "go" (CLAUDE.md autonomous execution).
