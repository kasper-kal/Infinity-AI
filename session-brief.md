LAST_UPDATED: 2026-09-09 — **GAP-FIX RUN: Phase 27 DONE (9 dimensions / 44 rules implemented) → next: P36** — Fixing all 42-phase-audit gaps autonomously per user ("could you start fixing them all, please dont go asking for my input. just... start."). P22 marketplace ✅, P33 automation tools ✅, P12 Unified Deploy ✅, P17 addImport/removeImport AST ops ✅, P15 skills panel ✅, P26 customization API mismatch ✅, P24 cursor panels ✅, P16 deploy+50 components ✅, P21 AnalyticsDashboard rendered ✅, P28 ComponentPlayground preview implemented ✅, **P27 9 dims / 44 rules ✅**. Remaining: P36 git-diff simulated + buildmap.* are routes not tools. Real, usable frontend auth system re: (`/api/auth/*`) — two-step MFA login, session elevation, persistent guest mode (`infinity-auth-guest`). Components:
...
- **2026-09-09 Phase 17 DONE — addImport/removeImport are now REAL AST operations** — Previously only string-typed in the frontend `useAstHistory.ts` AstOperation union with no backend behind them. Implemented in `artifacts/api-server/src/lib/ast-editor.ts`: `addImport(source, importDecl)` (parses the import statement, slots it right after existing import decls to keep the top of the file tidy, dedupes identical module source + specifier shape so re-adding is a no-op) and `removeImport(source, moduleName)` (backwards walk of program.body, removes every ImportDeclaration whose module specifier matches, strips surrounding quotes on input). Both return the standard `TransformResult { code, map, changes }` shape. Exposed via two new authenticated routes in `routes/infinity/ui-builder.ts`: `POST /api/infinity/ui-builder/ast/add-import` (zod: `{ code, import }`) and `POST /ast/remove-import` (zod: `{ code, module }`). Verified: esbuild bundle of both files exits 0 (ast-editor 18KB, ui-builder 1MB output) — full tsc/vite unavailable (no node_modules). Frontend already had the ops in the union type, so P17's "AST ops missing" gap is closed.
