LAST_UPDATED: 2026-09-08 — **Phase 41: File Format Conversion (@File Convert Command) — COMPLETE ✅** — Universal file format converter, fully local & $0: 35+ formats (documents, images, audio/video, data), magic-byte detection, @File chat command, full converter UI with single/batch drag-drop, route registration, EN+NL i18n. (Real local libs — file-type, marked/turndown, sharp, xlsx, ffmpeg-static, mammoth, pdfkit, pdf-parse — rather than Pandoc/LibreOffice WASM; Terminal CLI + right-click Convert deferred.)

## Change record (newest first)
- **2026-09-08 Phase 41 COMPLETE** — Verified existing implementation, finalized docs. Backend already existed: `file-converter.ts` (1057 lines), `wasm-modules.ts` (lazy local lib loaders), `file-convert.ts` routes (detect/convert/batch/info/formats), registered in infinity router. Frontend existed: FileConverter + FormatSelector + ConversionOptions + ConversionPreview + BatchQueue + index barrel (978 lines) + useFileConverter hook (235 lines). ChatView `@File` handler (Convert/ListFormats/Help). BuildView File Converter tab (bottomNav + Overview + command palette). 28 i18n keys EN+NL. Marked PHASES.md COMPLETE (overview table, checkboxes, current phase, upcoming). **Deferred:** TerminalView `infinity convert` CLI (simulated terminal, no real executor) + ProjectsView right-click Convert (no file-tree context menu).
