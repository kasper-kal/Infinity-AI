LAST_UPDATED: 2026-09-11 — **PHASE E COMPLETE — Green Must Mean Something (R3). ALL 5.1–5.6 DONE.** Success is earned, not labeled. **5.1/5.4** — `done` is only accepted by `evaluateDoneGate` after real file writes AND a live build+typecheck+test verification passes; rejections fed back as tool results; the route-level `runDoneContract` additionally downgrades `ok` on critical-gate failure in execute-plan. **5.2** — per-step quality gate in execute-plan (`runStepVerification` after each step's writes; a step is `completed` only when its own verify passes). **5.3** — living plan objects (`buildLivingPlan` + `livingSteps`) carry `{status, files[], verifyResult}`, persisted to checkpoints. **5.5** — the mind persists: real `finalPhase`, `editedFiles`, `lastDecision`, `tokenUsage`, `gates` saved; `buildResumeContext` injects them on resume. **5.6 FAILURE HONESTY** — canned plan → `503 { ok:false, plan:{...fallback:true} }`; plan gen retries transient LLM errors (RATE_LIMITED/QUOTA/SERVICE_UNAVAILABLE/TIMEOUT) with exponential backoff then fails loudly; agent routes return `ok` only from the real gate and propagate adapter/quota errors to a 500 (never `ok:true`). Hardcoded scaffold `phase:"planning"` → `agentResult.finalPhase`. Build clean (5.0s, only pre-existing context-compactor warnings). PHASES.md Phase E ✅ COMPLETE. Next: Phase F (The Model Sees Bytes, 3.1–3.3 + 7.1–7.20). — Earlier: PHASE D COMPLETE (world re-enters, false-green killed, DOM to iterate), PHASE C COMPLETE (loop is one mind, 6/6 counters), PHASE B COMPLETE (real workspaces + scaffold engine), PHASE A COMPLETE (10 env fixes), all 42 PHASES COMPLETE.
...
- **Current Phase:** **Phase E — Green Must Mean Something (R3)** ✅ **COMPLETE** — the 5 counters' mechanism is sound: `done` is gated on a real artifact + real verification, plans are only green when genuinely real, and fallback/quota/adapter failures always surface as `fallback:true` / `ok:false`. Next: **Phase F — The Model Sees Bytes (R2/R3)** (3.1–3.3, 7.1–7.20).
- **Roadmap status:** fix-campaign roadmap A–I live in PHASES.md — ✅ A (env fixes), ✅ B (real workspace + scaffold), ✅ C (loop is one mind), ✅ D (world comes back in), ✅ E (green means something), 🔲 F (model sees bytes) NOT STARTED, 🔲 G (error feedback loop), 🔲 H (preview+workspace context), 🔲 I (adaptive stop + real tests).
- **Phase 40 — Recipe Widget (Standard + Deep Research)** ✅ **COMPLETE**
- **Phase 39 — Enhanced LLM API Key System (Model Pickers, Task Categories, Build Modes)** ✅ **COMPLETE**
...
4c. **✅ DONE — PHASE E COMPLETE — Green Must Mean Something (R3)** — 5.1 done-gate + runDoneContract downgrade, 5.2 per-step quality gate, 5.3 living plan, 5.4 artifact+gate, 5.5 persist the mind, 5.6 failure honesty (fallback:true + never ok:true). **READY TO START — Phase F: The Model Sees Bytes** — per PHASES.md: 3.1 file bytes at decision points (read the step's declared files, inline token-capped contents), 3.2 repo context + token budget (git ls-files, package.json scripts/deps, README head, maxTokens 900→≥4000, drop 4 fixed dropdowns), 3.3 concise role tag (~500→~60 tokens), 7.1–7.20 design fixes tail; gate = captured coder request contains real file bytes; truncated file-map JSON can no longer produce a silent zero-write `ok`. Requires user "go" (CLAUDE.md autonomous execution).
