LAST_UPDATED: 2026-09-11 — **PHASE D COMPLETE — The World Comes Back In (R2) + FALSE-GREEN KILLED.** Feedback→iterate (4.2) now watches the LIVE preview (4.3): `/build/iterate` captures the running app's real DOM when a preview port is up, structured `{interactiveElements, visibleText, consoleErrors}` not Vite stdout; `/build/preview/agent` (4.4) returns the same structure + base64 screenshot; reviewer sees real file bytes, not `[Modified by step-X]` placeholders (4.5, build-orchestrator `applyCoderChanges` reads `readWorkspaceFileText`); dead prompt systems wired-or-deleted — `build-prompts.ts` deprecated (3.5); workspace conventions injected into coder system prompt — CLAUDE.md/AGENTS.md/.cursorrules/README + package.json scripts + toolchain config (6.2); component corpus named in prompt (6.3). **FALSE-GREEN bug killed:** `formatVerificationFeedback` returned "All checks passed." on genuine build failures — Fix 0.10's `failed:true` marker made `buildArtifacts.length>0` on failure, but the renderer gated Build-Failed on `length===0`; now renders every `failed` artifact's real stderr tail first. **Verified:** telemetry shows 0 "All checks passed" / 1 "Build Failed" on the broken-workspace gate run; `local_model_attempt` carries real TS errors (`lib/db/src/schema/tasks.ts`). Full Phase D gate: real exit codes, iterate reacts to broken `ok:false` (message: `Could not resolve entry module "index.html"`), verify_start events in telemetry. Build clean. PHASES.md Phase D ✅ COMPLETE. Next: Phase E (Green Must Mean Something). — Earlier: PHASE C COMPLETE (The Loop Is One Mind, 6/6 counters live-proven), PHASE B COMPLETE (Real Workspaces + Scaffold Engine), all 42 PHASES COMPLETE. `lib/build-agent.ts`: flat quality-gate loop replaces the 6-phase state machine; growing conversation (returns `conversation`, surfaced by `/build/agent/run`); native `completion.tool_calls` + regex fallback; stall detection; verify-after-edit; real state `{finalPhase, lastDecision, editedFiles, conversation}`. `done` + scaffold tools wired into Universal Tool Registry (fixed namespacing). `execute-plan`/agent/run use `runAutonomousAgent` tool-based coder; checkpoint saves real phase. **Live harness `/tmp/phase-c-harness.mjs` ALL PASS:** real account+project+user-api key+agent run → ok:true, iterations:3, finalPhase:"done", toolResults:3, conversation len:11, notes.txt on disk, verify sentinel present. **Bug fixed en route:** live `llm_keys.scopes` was `jsonb` vs `text[]` schema → api-key POST 500; migrated column in place (data preserved). Build clean (5.0s). PHASES.md Phase C ✅. Next: Phase D (The World Comes Back In). — Earlier: PHASE B COMPLETE (Real Workspaces + Scaffold Engine 6.4/6.4a/6.4b/6.4c, build-proven), PHASE A COMPLETE (all 10 env fixes), PASS 7 (LIVE PROOF), all 42 PHASES COMPLETE.
...
4. **✅ DONE — PHASE C COMPLETE — The Loop Is One Mind (R1)** — Live harness 6/6 counters PASS (ok:true, finalPhase:"done", toolResults non-empty, growing conversation len:11, files on disk, verify sentinel). New: **READY TO START — PHASE D: The World Comes Back In (R2)** — per PHASES.md: 4.1 real exit codes (remove ANY remaining `|| true`), 4.2 verification feedback → iterate, 4.3 preview agent in auto-pipeline (DOM findings as iterateGoal, not Vite stdout), 4.4 structured DOM output, 4.5 reviewer sees real file bytes, 3.5 wire-or-delete dead prompt systems (coderPromptV2/fixerPromptV2), 6.2 project conventions, 6.3 component corpus; gate = `verify_start` events appear, a broken file flips `ok` false, reviewer output reflects real code. Requires user "go" (CLAUDE.md autonomous execution).
