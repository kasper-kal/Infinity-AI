LAST_UPDATED: 2026-09-12 — **PHASE H COMPLETE — Preview + Workspace Context (R2). ALL H.1–H.6 DONE.** The model now sees what the user sees. **H.1** — `toolScreenshot` returns `imageDataUrl`; the loop strips raw base64 from the conversation/tool-history before it can pollute prompt text, and attaches the real image to the NEXT completion as an OpenAI-style `image_url` vision part (`detail:low`) on vision-capable adapters. Provider rejects the image (a free model that advertised vision but 400s)? `isVisionRejection` fails over to text-only ONCE and marks `visionDisabled` for the rest of the run — the loop never dies on an image. **H.2** — `capturePreviewDomForIterate` now also screenshots the running app in the same browser pass; the iterate goal gets `{interactiveElements, visibleText, consoleErrors, Screenshot: …}` and the screenshot is handed to `runAutonomousAgent` via new `opts.initialScreenshots` (vision part). Honest deviation (documented): the interactive LLM `/build/preview/agent` loop is NOT re-run per iterate (free-quota cost) — its structured readout reaches the model directly. **H.3** — Chrome deps installed + launch-verified on THIS host (chrome-headless-shell `--dump-dom` renders). **H.4** — `buildAgentSystemPrompt` injects `buildWorkspaceContentContext` (git ls-files tree + package.json scripts/deps + README/CLAUDE head + real file bytes) as `## WORKSPACE CONTENT`. **H.5** — conventions (`CLAUDE.md`/`.cursorrules`/README + scripts + toolchain) confirmed in system prompt. **H.6** — `list_files` result cached into `state.fileTree`; every user message carries `## WORKSPACE FILE TREE (persisted)`. Build clean (esbuild 3.2s, zero new warnings; bundle contains all markers). Next: Phase I (Adaptive Stop + Real Tests). — Earlier: PHASE G COMPLETE (Error Feedback Loop), PHASE F COMPLETE (The Model Sees Bytes), PHASE E COMPLETE (green must mean something, failure honesty), PHASE D COMPLETE (world re-enters, false-green killed, DOM to iterate), PHASE C COMPLETE (loop is one mind, 6/6 counters), PHASE B COMPLETE (real workspaces + scaffold engine), PHASE A COMPLETE (10 env fixes), all 42 PHASES COMPLETE.
...
- **2026-09-12 PHASE H COMPLETE — Preview + Workspace Context (R2)** — screenshot reaches the model as a real vision content part on the next turn with a text-only failover for free models that 400 on images (H.1); iterate auto-pipeline captures DOM + screenshot of the running app and hands it to the agent (H.2); Chrome deps installed + launch-verified on this host (H.3); workspace content (`buildWorkspaceContentContext` — tree, scripts, deps, README head, real file bytes) injected into every system prompt (H.4); conventions confirmed (H.5); list_files tree persists across iterations (H.6). Build clean (esbuild 3.2s, zero new warnings; bundle markers verified). PHASES.md + session-brief updated. Commit pushed. Next: Phase I (Adaptive Stop + Real Tests).
...
- **Current Phase:** **Phase G — Error Feedback Loop (R1+R2)** ✅ **COMPLETE** — verification failures become the next goal, fixers re-verify after applying patches (G.2), oscillation detection injects a strategy-change error (G.3), errors block is first-class (G.4), no `|| true` in verification (G.5). Next: **Phase H — Preview + Workspace Context** (H.1–H.6).
- **Roadmap status:** fix-campaign roadmap A–I live in PHASES.md — ✅ A (env fixes), ✅ B (real workspace + scaffold), ✅ C (loop is one mind), ✅ D (world comes back in), ✅ E (green means something), ✅ F (model sees bytes), ✅ G (error feedback loop), 🔲 H (preview+workspace context), 🔲 I (adaptive stop + real tests).
- **Phase 40 — Recipe Widget (Standard + Deep Research)** ✅ **COMPLETE**
- **Phase 39 — Enhanced LLM API Key System (Model Pickers, Task Categories, Build Modes)** ✅ **COMPLETE**
...
4e. **✅ DONE — PHASE H COMPLETE — Preview + Workspace Context** — H.1 screenshot→vision content part (image_url, detail:low) with text-only failover for free models; H.2 DOM + screenshot of the running app into the iterate auto-pipeline; H.3 Chrome deps installed + launch-verified on this host; H.4 `buildWorkspaceContentContext` in every system prompt; H.5 conventions (carried D 6.2) in system prompt; H.6 `list_files` tree persisted in `state.fileTree`. Build clean (esbuild 3.2s, zero new warnings). **READY TO START — Phase I: Adaptive Stop + Real Tests** — per PHASES.md: I.1 adaptive stop (quality-based, counter is backstop), I.2 real test execution (vitest/eslint/build, no `|| true`), I.3 per-step verification re-iterate, I.4 done contract wired as stop rule, I.5 fake gates marked `not-enforced`, I.6 quality-based stop conditions, I.7 coder writes tests as part of "done". Requires user "go" (CLAUDE.md autonomous execution).
