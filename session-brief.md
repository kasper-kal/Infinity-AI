...
- **2026-09-09 QUALITY CAMPAIGN — user: "the results aren't good" — pipeline audit + Fix 1/5 DONE** — User reframed: forget UI/UX (asked me to stop), and "the model is a smaller part of it than you think." Generation-pipeline audit (agent + direct reads) found: (1) thin prompts — no framework/version/convention/example context reaches the model, `infinity-prompt.ts` spends ~60 lines on identity and 0 on what good software is; (2) build-orchestrator Reviewer is a text-only LLM call that never runs tsc/tests/build; (3) `DoneContractEngine`/`runDoneContract` = dead code (zero callers) — no acceptance bar, "done" = agent's claim; (4) framework-generators/51-component corpus/template-engine imported by NO build route — model writes from scratch; (5) execute-plan path re-ran the same verify on failure without feeding errors to the model (`build.ts:1141`). **Fix 1 DONE: closed verification loop in `/build/execute-plan`** — on verify failure, `formatVerificationFeedback(verify)` now goes to a fixer LLM pass (`buildInfinityPrompt` role "fixer", jsonMode, withRetry) which writes repaired files then re-verifies, up to `maxRetries`; replaces the blind re-verify. api-server esbuild build ✅.
